...
// 조건을 추가하고 싶을 때, 특정 타입에 제한을 두고 싶을 때 등등 다양한 용도로 사용됩니다.
...
/*
  ...
  * 타입에 대한 제약 추가
*/
// 특정 패턴에 Bool 타입 조건을 지정하거나 어떤 타입의 특정 프로토콜 준수 조건을 추가하는 등의 기능이 있습니다.
...
for thing in things {
    switch thing {
        ...
        case is Double:
            print("sopme other double value that I don't want to print")
...
// 프로토콜 익스텐션에 where 절을 사용하면 이 익스텐션이 특정 프로토콜을 준수하는 타입에만 적용될 수 있도록 제약을 줄 수 있습니다.
// 익스텐션이 적용된 프로토콜을 준수하는 타입 중 where 절 뒤에 제시되는 프로토콜도 준수하는 타입만 익스텐션이 적용되도록 제약을 줄 수 있다는 뜻입니다.
...
// Element 타입이 Talkable 프로토콜을 준수하는 경우에만 Array 타입에 CallToAll 프로토콜을 채택했으므로
// Animal 타입을 요소(Element)로 갖는 Array 타입은 CallToAll 프로토콜을 채택하지 않습니다.
